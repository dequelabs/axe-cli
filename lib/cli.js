...
const axeTestUrls = require('./axe-test-urls')
const saveOutcome = require('./save-outcome')
const utils = require('./utils')

module.exports = function axeCliRunner (process, console) {
  ...
  program.axeSource = (program.axeSource)

  if (!program.axeSource) {
    const message = 'Unable to find the axe-core source file.'
  ...
  return testUrls(urls, program, console)
}

function testUrls (process, console, urls, program) {
  let cliReporter
  ...
  axeTestUrls(urls, program, {
    ...
    onTestComplete: function logResults (results) {
      ...
      const issueCount = violations.reduce((count, violation) => {
        const errorMessage = error('  Violation of %j with %d occurrences!\n') +
